...
function Inspector(divid, diagram, options) {
  var mainDiv = divid;//document.getElementById(divid);
...
Inspector.prototype.inspectObject = function(object) {
    ...
    if(data.hasOwnProperty("dataInsectorPlugin") == true)
        var plugin = _plugins[data['dataInsectorPlugin']];
    else if(data.hasOwnProperty("category"))
        var plugin = _plugins[data['category']];
    else
        var plugin = null;
    ...
    for (var k in declaredProperties) {
        ...
        tbody.appendChild(this.buildPropertyRow(k, defaultValue || "", plugin));
    }
    ...
    if (this.includesOwnProperties) {
        for (var k in data) {
            ...
            tbody.appendChild(this.buildPropertyRow(k, data[k],plugin));
        }
    }
...
* @param plugin: option ref to the plugin object, for getting the selection list (dropdown) for fields
* @return the table row
*/
Inspector.prototype.buildPropertyRow = function(propertyName, propertyValue, plugin) {
    var mainDiv = this._div;
    ...
    function processInput() {
        ...
        var decProp = self.declaredProperties[propertyName];
        input.tabIndex = self.tabIndex++;


        if ((propertyValue.constructor == {}.constructor) || propertyValue.constructor == [].constructor) { //it's a json object, so stringify
            input.value = JSON.stringify(propertyValue);
        }
        else
            input.value = propertyValue;
        input.disabled = !self.canEditProperty(propertyName, decProp, self.inspectedObject);
        if (decProp !== undefined && decProp.type === "color") {
            input.setAttribute("type", "color");
            if (input.type === "color") {
                input.addEventListener("input", setprops);
                input.addEventListener("change", setprops);
                input.value = self.setColor(propertyValue);
            }
        }
        ...
        if (self._diagram.model.isReadOnly) input.disabled = true;

        if (input.type !== "color") input.addEventListener("blur", setprops);
        return input;
    }

    function processSelect(options, context){
        ...
        for(var i = 0; i < options.length; i++){
            ...
            item.text = options[i];
            input.options.add(item, i);
        }
        input.selectedIndex = options.indexOf(propertyValue);
        input.addEventListener("input", setprops);
        input.addEventListener("change", setprops);
    ...
    var input = null;
    var didSelect = false;
    if(plugin && plugin.hasOwnProperty("getSelectionForField")){
        var options = plugin.getSelectionForField(propertyName);
        if(options && options.length > 0 ){
            input = processSelect(options);
            didSelect = true;
        }
    }
    if(didSelect == false)
...
Inspector.prototype.updateAllProperties = function() {
    ...
    for (var name in inspectedProps) {
        if (inspectedProps[name].type == "select-one")
            var value = inspectedProps[name].options[inspectedProps[name].selectedIndex].label;
        else if (inspectedProps[name].type == "checkbox")
        ...
        //but not if it's a checkbox, then the value is correctly shown.
        if (inspectedProps[name].type != "checkbox")
